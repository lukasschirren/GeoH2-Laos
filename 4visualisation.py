...
def process_scenario(hydro_year, electrolyser_type, scenario_year, generation_type='total'):
    ...
    try:
        ...
        generate_waterfall_chart(gdf_data, 'trucking',
                               os.path.join(scenario_folder, 'waterfall_chart.png'))
        
        # Generate capacity maps
        capacity_types = ['hydro', 'solar']  # Removed wind as not used
        for capacity_type in capacity_types:
            plot_capacity_map(
                ...
                vmin=20 if capacity_type != 'hydro' else 0,
                vmax=440 if capacity_type != 'hydro' else 100
            )
